...
const Card = (props) => {
    ...
    return (
        <div className=' w-full shrink-0  md:w-[260px]  mb-3'>
            <div className='group h-[180px] relative overflow-hidden rounded-[15px] '>
                <img className='group-hover:scale-110 duration-150 object-cover h-full w-full rounded-[15px]' src={props.image} alt="" />
                <div className='image-overlay absolute w-full h-full top-0 flex items-end p-2 text-[16px]    md:text-xl text-white font-bold rounded-b-[15px]'>{props.offer}</div>
            </div>
            <div className='mt-1 text-[14px] md:text-[18px] font-bold'>
                {props.title}
            </div>
            <div>
                <Star className="inline"/>{props.rating} <span className='ml-6 text-stone-700 text-[14px] md:text-[18px]'>{props.minTime}-{props.maxTime} mins</span>
            </div>
            <div className='text-stone-700 text-[14px] md:text-[18px]'>
                {props.name}
                <br/>
                {props.place}
            </div>
